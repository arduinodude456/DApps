...
[`dock_update.lua`](dock_update.lua) ist die **manuelle Kernaktualisierung** für AppDock. In Version **1.0.2** ignoriert DockUpdate Markdown-Dokumentation wie `README.md` und den zusätzlichen verpackten `appdock.koplugin/`-Spiegel im GitHub-Quellbaum; nur die erwarteten AppDock-Lua-Kernmodule aus dem Root gelangen in die Validierung und den atomaren Updatepfad. Mit **Check updates** fragt sie ausschließlich den neuesten stabilen Release von [`arduinodude456/appdock.koplugin`](https://github.com/arduinodude456/appdock.koplugin) über HTTPS ab. Die Karte zeigt installierte und veröffentlichte Version, eine Zusammenfassung der Release Notes sowie den Updatestatus; **Release Notes** öffnet den vollständigen, lesbaren Release-Text.
